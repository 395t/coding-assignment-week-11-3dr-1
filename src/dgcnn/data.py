...
def download_modelnet(num_classes):
    BASE_DIR = os.path.dirname(os.path.abspath(__file__))
    DATA_DIR = os.path.join(BASE_DIR, 'data')
    if not os.path.exists(DATA_DIR):
        os.mkdir(DATA_DIR)
    if num_classes == 10:
        filename = 'modelnet10_hdf5_2048'
        www = 'https://www.dropbox.com/s/d5tnwg2legbd6rh/modelnet10_hdf5_2048.zip?dl=1'
    else:
        filename = 'modelnet40_ply_hdf5_2048'
        www = 'https://shapenet.cs.stanford.edu/media/modelnet40_ply_hdf5_2048.zip'
    if not os.path.exists(os.path.join(DATA_DIR, filename)):
        zipfile = os.path.basename(www)
        os.system('wget %s --no-check-certificate; unzip %s' % (www, zipfile))
        os.system('mv %s %s' % (filename, DATA_DIR))
...
def load_data_cls(num_classes, partition):
    download_modelnet(num_classes)
    if num_classes == 10:
        filename = 'modelnet10_hdf5_2048'
    else:
        filename = 'modelnet40_ply_hdf5_2048'
    ...
    all_data = []
    all_label = []
    for h5_name in glob.glob(os.path.join(DATA_DIR, filename, '*%s*.h5'%partition)):
...
class ModelNet(Dataset):
    def __init__(self, num_classes, num_points, partition='train'):
        self.data, self.label = load_data_cls(num_classes, partition)
...
if __name__ == '__main__':
    train = ModelNet(40, 1024)
    test = ModelNet(40, 1024, 'test')
    data, label = train[0]
    print(data.shape)
    print(label.shape)
    ...
    train = ModelNet(10, 1024)
    test = ModelNet(10, 1024, 'test')
    data, label = train[0]
    print(data.shape)
    print(label.shape)
